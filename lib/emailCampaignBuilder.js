...
export function rewriteEmailPreviewMediaUrls(html = '', origin = '') {
  ...
  return String(html).replace(/<img\b([^>]*)>/gi, (full, attrs) => {
    ...
    if (src.startsWith('//')) {
      ...
      // Bind local public assets to the current preview origin (localhost-safe).
      src = `${base || 'https://store1920.com'}${src}`;
    } else if (!/^https?:\/\//i.test(src)) {
      const absolute = toAbsoluteMediaUrl(src);
      if (absolute) src = absolute;
    }

    // Email logo often uses NEXT_PUBLIC_BASE_URL; in local preview use this origin's public file.
    if (base && /\/logo\/Store1920\.png(?:\?|$)/i.test(src)) {
      src = `${base}/logo/Store1920.png`;
    }
...
function renderCardMedia({
  ...
  const mediaInner = image
    ? (fixedHeight
      ? `<div style="width:100%;height:${fixedHeight}px;overflow:hidden;background:#f8fafc;line-height:0;">
          <img src="${escapeHtml(image)}" alt="" width="600" height="${fixedHeight}" ${imgAttrs} style="width:100%;height:${fixedHeight}px;object-fit:${objectFit};object-position:center;display:block;border:0;outline:none;text-decoration:none;background:#f8fafc;" />
        </div>`
      : `<div style="width:100%;padding-bottom:${paddingPercent}%;height:0;position:relative;overflow:hidden;background:#f8fafc;line-height:0;">
          <img src="${escapeHtml(image)}" alt="" ${imgAttrs} style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:${objectFit};object-position:center;display:block;border:0;outline:none;background:#f8fafc;" />
        </div>`)
    : (fixedHeight
