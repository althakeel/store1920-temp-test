...
const BASE_URL = () => getCustomerSiteUrl() || 'https://www.store1920.com';
const CURRENCY = () => process.env.NEXT_PUBLIC_CURRENCY_SYMBOL || 'AED';

/** Pasted custom HTML is a complete email (doctype/html/body), not a section fragment. */
...
/** Preview iframes must not wrap a full document inside another html/body. */
export function toEmailPreviewSrcDoc(html = '', { background = '#e2e8f0', padding = '' } = {}) {
  const raw = String(html || '');
  if (!raw) return raw;
  if (isFullEmailDocument(raw)) return raw;
  const pad = padding ? `padding:${padding};` : '';
  return `<!DOCTYPE html><html><body style="margin:0;background:${background};${pad}">${raw}</body></html>`;
}
...
export function absolutizeEmailHtmlImages(html = '') {
  if (!html) return html;
  // Fix curly quotes so every href= is parseable by email clients.
...
function renderDetailCard(product, options = {}) {
  ...
  const productUrl = getProductAbsoluteUrl(product, baseUrl) || `${baseUrl}/shop`;
...
function renderFooter(block = {}, recipientEmail = '') {
  const baseUrl = safeHref(block.companyWebsite).replace(/\/$/, '') || 'https://www.store1920.com';
...
function readHtmlAttr(attrs, name) {
  const source = String(attrs || '');
  const quoted = source.match(new RegExp(`\\s${name}\\s*=\\s*(["'])([\\s\\S]*?)\\1`, 'i'));
  if (quoted) return quoted[2];
  const unquoted = source.match(new RegExp(`\\s${name}\\s*=\\s*([^\\s>]+)`, 'i'));
  return unquoted ? unquoted[1] : '';
}

function collectEmailDocumentHead(html = '') {
  const head = String(html || '').match(/<head\b[^>]*>([\s\S]*?)<\/head>/i)?.[1] || '';
  const title = String(head.match(/<title\b[^>]*>([\s\S]*?)<\/title>/i)?.[1] || '')
    .replace(/<[^>]+>/g, '')
    .trim();
  const metas = [];
  const re = /<meta\b[^>]*>/gi;
  ...
  while ((match = re.exec(head))) {
    ...
    if (/charset|viewport|x-apple-disable-message-reformatting|format-detection|color-scheme|supported-color-schemes/i.test(tag)) {
      metas.push(tag);
    }
  }
  return { title, metas };
 ...
 * If a full document is pasted, keep body styles so send matches the design.
 */
function prepareSanitizedEmailHtml(html = '', extraCss = '') {
  let raw = String(html || '');
  const isFullDocument = isFullEmailDocument(raw);
  const { title, metas } = collectEmailDocumentHead(raw);
  ...
  if (bodyMatch) {
    bodyStyle = readHtmlAttr(bodyMatch[1], 'style');
    bodyBgcolor = readHtmlAttr(bodyMatch[1], 'bgcolor');
    raw = bodyMatch[2];
  }
  ...
  return {
    ...
    bodyStyle,
    bodyBgcolor,
    body: rewriteEmailHtmlFragmentUrls(raw),
    css: collectedCss.join('\n'),
  };
}

function sanitizeEmailHtmlFragment(html = '', { extraCss = '', inlineCss = false } = {}) {
  const prepared = prepareSanitizedEmailHtml(html, extraCss);
  const styleBlock = prepared.css
    ? `<style type="text/css">${prepared.css}</style>`
    : '';

  if (!prepared.body && !styleBlock) return '';

  if (prepared.isFullDocument) {
    const style = prepared.bodyStyle || (prepared.bodyBgcolor ? `background:${prepared.bodyBgcolor};` : '');
    const inner = `${styleBlock}${prepared.body}`;
    const fragment = style
      ? `<div class="email-html-document" style="${escapeHtml(style)}">${inner}</div>`
      : inner;
    return inlineCss ? inlineEmailCss(fragment) : fragment;
  }

  const fragment = `${styleBlock}${prepared.body ? `<div class="email-html-block">${prepared.body}</div>` : ''}`;
  return inlineCss ? inlineEmailCss(fragment) : fragment;
}
...
function findStandaloneHtmlBlock(blocks = []) {
  return (Array.isArray(blocks) ? blocks : []).find((block) => (
    block?.type === 'html' && isFullEmailDocument(block.html)
  )) || null;
}

function defaultStandaloneMetas() {
  return [
    '<meta charset="utf-8">',
...
/** Send a pasted full email document as the message itself — no 620px builder chrome. */
function buildStandaloneEmailDocument(block = {}, { previewMode = false, preheader = '' } = {}) {
  const prepared = prepareSanitizedEmailHtml(block.html, block.css || '');
  const ctaHtml = renderHtmlBlockCta(block);
  let inner = prepared.body || '';
  if (ctaHtml) {
    inner = block.ctaPosition === 'above' ? `${ctaHtml}${inner}` : `${inner}${ctaHtml}`;
  }

  const withCss = prepared.css
    ? `<style type="text/css">${prepared.css}</style>${inner}`
    : inner;
  const rendered = previewMode ? withCss : inlineEmailCss(withCss);

  const pulledCss = [];
  const bodyHtml = String(rendered || '').replace(/<style\b[^>]*>([\s\S]*?)<\/style>/gi, (_, css) => {
    if (css) pulledCss.push(css);
    return '';
  });

  const bodyStyle = prepared.bodyStyle
    || 'margin:0;padding:0;width:100%;-webkit-text-size-adjust:100%;background:#f3f4f7;';
  const metas = prepared.metas.length ? prepared.metas : defaultStandaloneMetas();
  const title = prepared.title || 'Store1920';
  const campaignPreheader = String(preheader || '').trim();
  const hiddenPreheader = campaignPreheader
    ? `<div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent">${escapeHtml(campaignPreheader)}</div>`
    : '';
...
${pulledCss.length ? `<style type="text/css">${pulledCss.join('\n')}</style>` : ''}
</head>
<body${prepared.bodyBgcolor ? ` bgcolor="${escapeHtml(prepared.bodyBgcolor)}"` : ''} style="${escapeHtml(bodyStyle)}">
${hiddenPreheader}
${bodyHtml}
</body>
</html>`;
...
function renderBlock(block, { products = [], recipientEmail = '', previewMode = false } = {}) {
  switch (block.type) {
    ...
    case 'text':
      ...
      `;
    case 'html': {
      if (isFullEmailDocument(block.html)) {
        return sanitizeEmailHtmlFragment(block.html, {
          extraCss: block.css || '',
          inlineCss: !previewMode,
        });
      }
      const fragment = sanitizeEmailHtmlFragment(block.html, {
        extraCss: block.css || '',
        inlineCss: !previewMode,
      });
      const ctaHtml = renderHtmlBlockCta(block);
...
export function renderEmailFromBlocks(blocks = [], {
  ...
  const standalone = findStandaloneHtmlBlock(list);
  if (standalone) {
    let doc = buildStandaloneEmailDocument(standalone, {
      previewMode: isPreview,
      preheader,
    });
    if (interactivePreview) {
      const blockId = standalone.id || 'html-standalone';
      doc = doc.replace(
        /<body\b([^>]*)>/i,
        `<body$1><div data-block-id="${escapeHtml(blockId)}" class="email-preview-block" role="button" tabindex="0">`,
      );
      doc = doc.replace(/<\/body>/i, '</div></body>');
    }
    if (isPreview) return doc;
    return wrapMarketingEmailDocument(absolutizeEmailHtmlImages(doc));
  }
  ...
  return wrapMarketingEmailDocument(absolutizeEmailHtmlImages(html));
}
