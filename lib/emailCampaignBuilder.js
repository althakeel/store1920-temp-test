...
function renderCardMedia({
  ...
  const mediaInner = image
    ? `<div style="width:100%;padding-bottom:${paddingPercent}%;height:0;position:relative;overflow:hidden;background:#ffffff;">
        <img src="${escapeHtml(image)}" alt="${escapeHtml(product.name || 'Product')}" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:${objectFit};object-position:center;display:block;" />
      </div>`
    : `<div style="width:100%;padding-bottom:${paddingPercent}%;height:0;background:#f3f4f6;"></div>`;
  return `
    <a href="${escapeHtml(link)}" ${newTabLinkAttrs()} style="display:block;position:relative;text-decoration:none;background:#ffffff;">
      ${mediaInner}
      ${videoMode ? `
      ...
      ` : ''}
      ${showBadge ? `<span style="position:absolute;top:10px;left:10px;background:#111827;color:#fff;font-size:10px;font-weight:800;letter-spacing:0.6px;padding:5px 8px;border-radius:999px;">${escapeHtml(badgeText)}</span>` : ''}
      ${showDiscountPercent && saving > 0 ? `<span style="position:absolute;top:10px;right:10px;background:${escapeHtml(accent)};color:#fff;font-size:10px;font-weight:800;padding:5px 8px;border-radius:999px;">-${saving}%</span>` : ''}
    </a>
  `;
}
...
function renderDetailCard(product, options = {}) {
  ...
  const mediaOpts = {
    ...
    saving, showDiscountPercent, accent: btnColor,
  };
    ...
    return `
      <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
        <div style="${cardShellStyle({ ...shell, radius: 12 })}">
          ${renderCardMedia({ ...mediaOpts, height: 170, showBadge: false, showDiscountPercent: false, fit: 'contain' })}
    ...
    return `
      <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
        <div style="${cardShellStyle({ ...shell, borderColor: btnColor, showBorder: true, radius: 16 })}border:2px solid ${escapeHtml(btnColor)};">
          ${renderCardMedia({ ...mediaOpts, height: 180, showDiscountPercent: false, fit: 'contain' })}
    ...
    return `
      <td style="width:${widthPercent}%;padding:6px;vertical-align:top;">
        <div style="${cardShellStyle({ ...shell, radius: 10 })}">
          ${renderCardMedia({ ...mediaOpts, height: 120, showDiscountPercent: false, fit: 'contain' })}
      ...
      <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
        <div style="${cardShellStyle({ ...shell, bg: bg === '#ffffff' ? '#f8fafc' : bg, radius: 24, showBorder: false })}padding:10px;">
          <div style="border-radius:18px;overflow:hidden;">
            ${renderCardMedia({ ...mediaOpts, height: 170, showDiscountPercent: false, fit: 'contain' })}
          </div>
  ...
  return `
    <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
      <div style="${cardShellStyle({ ...shell, radius: 14 })}">
        ${renderCardMedia({
          ...mediaOpts,
          height: widthPercent >= 100 ? 220 : 180,
