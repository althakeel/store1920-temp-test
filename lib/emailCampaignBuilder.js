...
/** Final pass before send: absolute https image URLs (no lazy-load). */
...
function rewriteAttrUrl(attrs, attrName, rewriter) {
  const pattern = new RegExp(`\\s${attrName}\\s*=\\s*(["'])([\\s\\S]*?)\\1`, 'i');
  const match = String(attrs || '').match(pattern);
  if (!match) return attrs;
  const next = rewriter(String(match[2] || '').replace(/&amp;/g, '&').trim());
  if (!next) return attrs;
  const safe = next.replace(/&/g, '&amp;').replace(/"/g, '&quot;');
  return String(attrs).replace(pattern, ` ${attrName}="${safe}"`);
}
