...
const safeHref = (url) => normalizeCustomerUrl(url || BASE_URL()) || BASE_URL();
...
/** Preview iframes can't reliably load CDN images — proxy through our API. */
export function rewriteEmailPreviewMediaUrls(html = '', origin = '') {
  const base = String(origin || '').replace(/\/$/, '');
  if (!base || !html) return html;
  return String(html).replace(/(\s(?:src)=["'])([^"']+)(["'])/gi, (full, pre, src, post) => {
    const decoded = String(src || '').replace(/&amp;/g, '&').trim();
    if (!decoded || decoded.startsWith('data:') || decoded.includes('/api/store/email-marketing/media')) {
      return full;
    }
    const absolute = toAbsoluteMediaUrl(decoded) || decoded;
    if (!/^https?:\/\//i.test(absolute)) return full;
    const proxied = `${base}/api/store/email-marketing/media?url=${encodeURIComponent(absolute)}`;
...
function pickProducts(products = [], block = {}) {
  ...
  const mode = block.selectionMode || (
    Array.isArray(block.productIds) && block.productIds.length ? 'manual' : 'auto'
  );
  ...
  if (mode === 'manual') {
    ...
    if (ids.length) {
      ...
      if (chosen.length) return chosen.slice(0, Math.max(limit, ids.length));
    }
  }
  ...
  if (mode === 'category' || block.category) {
    ...
    if (category) {
      const filtered = list.filter((item) => String(item.category || '').toLowerCase() === category);
...
function productImage(product) {
  // Prefer a real photo — first media slot is often a video and breaks <img> cards.
  const thumb = getProductThumbnailUrl(product, { fallback: '', allowVideo: false });
  if (thumb) return toAbsoluteMediaUrl(thumb);
  const legacy = product?.image || (Array.isArray(product?.images) ? product.images[0] : '') || '';
  return toAbsoluteMediaUrl(legacy);
}
...
function renderCardMedia({
  ...
  const mediaInner = image
    ? (fixedHeight
      ? `<div style="width:100%;height:${fixedHeight}px;overflow:hidden;background:#f8fafc;position:relative;">
          <img src="${escapeHtml(image)}" alt="${escapeHtml(product.name || 'Product')}" width="300" height="${fixedHeight}" referrerpolicy="no-referrer" style="width:100%;height:${fixedHeight}px;object-fit:${objectFit};object-position:center;display:block;background:#f8fafc;" />
        </div>`
      : `<div style="width:100%;padding-bottom:${paddingPercent}%;height:0;position:relative;overflow:hidden;background:#f8fafc;">
          <img src="${escapeHtml(image)}" alt="${escapeHtml(product.name || 'Product')}" referrerpolicy="no-referrer" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:${objectFit};object-position:center;display:block;background:#f8fafc;" />
        </div>`)
    : (fixedHeight
  ...
  return `
    <a href="${escapeHtml(link)}" ${newTabLinkAttrs()} style="display:block;position:relative;text-decoration:none;background:#f8fafc;">
      ${mediaInner}
      ${videoMode ? `
...
function renderPriceHtml({
  ...
  // One consistent row for every card — never wrap % onto a second line alone.
  return `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:10px;border-collapse:collapse;">
      <tr>
        <td style="text-align:${align};vertical-align:middle;white-space:nowrap;line-height:1.2;">
          ${sale ? `<span style="font-size:${size}px;font-weight:800;color:${escapeHtml(accent)};">${currency}${Math.floor(salePrice)}</span>` : ''}
          ${original ? `<span style="margin-left:6px;font-size:12px;color:#9ca3af;text-decoration:line-through;">${currency}${Math.floor(originalPrice)}</span>` : ''}
          ${percent ? `<span style="margin-left:6px;display:inline-block;font-size:10px;font-weight:800;color:#ffffff;background:#dc2626;padding:2px 6px;border-radius:999px;vertical-align:middle;">-${saving}%</span>` : ''}
        </td>
      </tr>
    </table>
  `;
...
function renderEmptyProducts(label = 'products') {
  return `
    <div style="padding:28px 16px;text-align:center;color:#6b7280;font-size:13px;border:1px dashed #d1d5db;border-radius:12px;background:#fff;">
      Real ${escapeHtml(label)} with images will appear here when you send (or when preview products load).
    </div>
  `;
}
...
function renderDetailCard(product, options = {}) {
  ...
  if (style === 'minimal') {
    return `
      <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
  ...
  if (style === 'compact') {
    return `
      <td style="width:${widthPercent}%;padding:6px;vertical-align:top;">
        <div style="${cardShellStyle({ ...shell, radius: 10 })}">
          ${renderCardMedia({ ...mediaOpts, height: ratioHeight(120), showDiscountPercent: false, fit: 'cover' })}
          <div style="padding:10px;">
            ${renderProductTitle(product.name, { lines: 2, fontSize: 12 })}
            ${renderPriceHtml({ ...priceOpts, size: 14 })}
    ...
    return `
      <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
        <div style="${cardShellStyle({ ...shell, radius: 4, showBorder: false })}border-bottom:3px solid ${escapeHtml(btnColor)};">
          ${renderCardMedia({ ...mediaOpts, height: widthPercent >= 100 ? 260 : 200 })}
  ...
  if (style === 'overlay') {
    const height = widthPercent >= 100 ? 280 : 220;
    const videoMode = mediaMode === 'video';
    const { thumb, href } = productVideoMeta(product, productUrl);
    const image = videoMode ? thumb : productImage(product);
    const link = videoMode ? href : productUrl;
    return `
      <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
        <a href="${escapeHtml(link)}" style="display:block;text-decoration:none;border-radius:16px;overflow:hidden;${showShadow === false ? '' : 'box-shadow:0 10px 28px rgba(15,23,42,0.18);'}">
          <div style="position:relative;height:${height}px;background:#0f172a;">
            ${image ? `<img src="${escapeHtml(image)}" alt="" style="width:100%;height:${height}px;object-fit:cover;display:block;" />` : ''}
  ...
  // classic — uniform card for grids
  return `
    <td style="width:${widthPercent}%;padding:8px;vertical-align:top;">
      <div style="${cardShellStyle({ ...shell, radius: 14 })}">
        ${renderCardMedia({
          ...mediaOpts,
          height: ratioHeight(widthPercent >= 100 ? 220 : 180),
          showDiscountPercent: false,
          fit: 'cover',
        })}
        <div style="padding:14px;">
          ${category ? `<div style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:0.4px;margin-bottom:6px;height:14px;overflow:hidden;">${escapeHtml(category)}</div>` : ''}
          ${renderProductTitle(product.name, { lines: 2, fontSize: 14 })}
          ${description ? `<div style="margin-top:6px;font-size:12px;color:#6b7280;line-height:1.45;height:34px;overflow:hidden;">${escapeHtml(description)}</div>` : ''}
          ${renderPriceHtml({ ...priceOpts, size: 18 })}
          ${renderCtaHtml(ctaOpts)}
        </div>
      </div>
...
function renderProductGridLayout(products, block, options = {}) {
  ...
  const rowsWanted = Math.min(6, Math.max(1, Number(block.gridRows) || 2));
  const maxFromRows = rowsWanted * columns;
  const list = products.slice(0, Math.min(products.length, Math.max(Number(block.limit) || 4, maxFromRows)));
  const capped = list.slice(0, maxFromRows);
  ...
  const cell = (title, html, image) => `
    <td style="width:50%;padding:8px;vertical-align:top;">
      <div style="background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
        ${image ? `<img src="${escapeHtml(image)}" alt="" style="width:100%;height:160px;object-fit:cover;display:block;" />` : ''}
        <div style="padding:14px;">
...
function renderCarouselLayout(products, block) {
  ...
  const cells = products.map((product) => {
    ...
    return `
      <td style="padding:0 6px;vertical-align:top;width:168px;min-width:168px;max-width:168px;">
        <div style="${cardShellStyle({
          bg: block.cardBackground || '#ffffff',
          borderColor: block.borderColor || '#e5e7eb',
          showBorder: block.showBorder !== false,
          showShadow: block.showShadow !== false,
          radius: 16,
          ...
          ${renderCardMedia({
            product,
            productUrl,
            height: Math.round(168 * ((block.imageRatio === '4:5') ? 1.25 : 1)),
            imageRatio: block.imageRatio || '1:1',
            mediaMode: block.mediaMode || 'image',
            showBadge: Boolean(block.showBadge),
            badgeText: block.badgeText || 'NEW',
            saving: meta.saving,
            // % lives only in the price row so every card matches
            showDiscountPercent: false,
            accent,
            fit: 'cover',
          })}
...
function renderProductSection(block, products, layout) {
  ...
  const modeLabel = block.selectionMode === 'manual'
    ? 'Chosen products'
    : block.selectionMode === 'category' && block.category
  ...
  return `
    <div style="padding:8px 18px 24px;background:#f8fafc;">
      ...
      <p style="margin:0 0 14px;text-align:center;font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:0.8px;">${escapeHtml(modeLabel)}</p>
      ${body}
    </div>
  `;
}
...
function renderFooter(block = {}, recipientEmail = '') {
  ...
  const logoHtml = logoSrc
    ...
    : emailLogoImg(`max-width:${logoWidth}px;height:auto;display:inline-block;`);
  ...
  return `
    <div style="background:${escapeHtml(block.backgroundColor || '#0f172a')};padding:32px 24px;text-align:${position};color:#cbd5e1;">
      ${block.showLogo !== false ? `<div style="margin-bottom:12px;">${logoHtml}</div>` : ''}
...
function renderBlock(block, { products = [], recipientEmail = '' } = {}) {
  switch (block.type) {
    case 'header': {
      ...
      const logoImg = logoSrc
        ...
        : emailLogoImg(imgStyle);
      const logoHtml = `<a href="${escapeHtml(logoHref)}" style="text-decoration:none;display:inline-block;">${logoImg}</a>`;
      ...
      if (imageUrl) {
        return `
          <div style="position:relative;background:#0f172a;">
            <img src="${escapeHtml(imageUrl)}" alt="" style="width:100%;height:280px;object-fit:cover;display:block;opacity:${block.overlay === false ? '1' : '0.55'};" />
      ...
      return `
        <div style="padding:12px 28px;background:#f8fafc;">
          <a href="${escapeHtml(safeHref(block.url))}">
            <img src="${escapeHtml(block.src)}" alt="${escapeHtml(block.alt || '')}" style="width:100%;max-width:100%;border-radius:12px;display:block;" />
          </a>
        </div>
      `;
    case 'products':
      return renderProductSection(block, products, 'grid');
    case 'product_carousel':
      return renderProductSection(block, products, 'carousel');
    case 'product_latest':
      return renderProductSection({ ...block, productSource: block.productSource || 'latest' }, products, 'latest');
    case 'product_related':
      return renderProductSection({ ...block, productSource: block.productSource || 'related' }, products, 'related');
    case 'divider':
      return `
...
function renderFormBlock(block = {}) {
  ...
  const href = escapeHtml(safeHref(block.successUrl || BASE_URL()));
  ...
  const note = 'Signup opens on your site — email clients cannot submit forms directly.';
...
export function renderEmailFromBlocks(blocks = [], {
  ...
  const body = list.map((block, index) => {
    const html = renderBlock(block, { products, recipientEmail });
  ...
  const previewStyles = interactivePreview ? `
        .email-preview-block{cursor:pointer;outline:2px solid transparent;outline-offset:-2px;transition:outline-color .15s,box-shadow .15s;}
        .email-preview-block:hover{outline-color:#5eead4;}
        .email-preview-block.is-selected{outline-color:#0f766e;box-shadow:inset 0 0 0 9999px rgba(15,118,110,0.05);}
      ` : '';
  return `
