...
export async function approveReturnRequestAndCreateFollowUp({
  ...
  try {
    ...
    if (reloaded) {
      const synced = await syncWaslahReturnTrackingForOrder(reloaded, { persist: true });
      if (synced?.order?.waslahReturn) {
        pickupShipResult = {
          ...pickupShipResult,
          trackingNumber: synced.order.waslahReturn.trackingNumber || pickupShipResult.trackingNumber,
          waslahOrderId: synced.order.waslahReturn.orderId || pickupShipResult.waslahOrderId,
          originalTrackingNumber: synced.order.waslahReturn.originalTrackingNumber || null,
        };
      }
    }
  } catch (syncError) {
    console.warn('[return-follow-up] return tracking sync failed:', syncError?.message || syncError);
  }
  if (
  ...
  return {
    ...
    order: await Order.findById(order._id)
      .select('_id status returns waslah waslahReturn trackingId shortOrderNumber')
      .lean(),
  };
}
