...
function getOrderShippingFee(order = {}) {
  const fee = Number(order.shippingFee ?? order.shipping ?? 0);
  return Number.isFinite(fee) && fee > 0 ? fee : 0;
}

function getShippingItemSku() {
  return String(process.env.ZOHO_INVENTORY_SHIPPING_SKU || 'STORE1920-SHIPPING').trim();
}

let cachedShippingItemId = null;

async function ensureShippingServiceItem() {
  if (cachedShippingItemId) return cachedShippingItemId;

  const sku = getShippingItemSku();
  const existing = await findInventoryItemBySku(sku);
  if (existing?.item_id) {
    cachedShippingItemId = String(existing.item_id);
    return cachedShippingItemId;
  }
  ...
  const created = await zohoInventoryRequest('/items', {
    ...
    body: {
      ...
      can_be_sold: true,
      description: 'Store1920 shipping',
    },
  });
  const itemId = created?.item?.item_id || created?.items?.[0]?.item_id || null;
  if (!itemId) return null;
  cachedShippingItemId = String(itemId);
  return cachedShippingItemId;
}
...
async function buildLineItems(order = {}, locationId = null) {
  ...
  if (shippingFee > 0) {
    ...
    try {
      const shippingItemId = await ensureShippingServiceItem();
      if (shippingItemId) shippingLine.item_id = shippingItemId;
    } catch (shippingItemError) {
      console.warn(
        '[zoho-inventory] shipping service item skipped:',
        shippingItemError?.message || shippingItemError,
      );
    }
...
async function createInventorySalesOrder(order = {}, customerId) {
  const locationId = await resolveInventoryLocationId();
  const lineItems = await buildLineItems(order, locationId);
  if (!lineItems.length) {
