...
export function toEmailAudienceRecipient(customer) {
  const promotionalOptOut = customer.promotionalOptOut === true
    || customer.emailPreferences?.promotional === false;
  const email = String(customer.email || '').trim().toLowerCase();
  // Prefer email as the stable selection key — userIds can collide/duplicate in the list
  // and break multi-select (second click toggles the same key off).
  const id = email.includes('@')
    ? email
...
/** Keep one row per email so checkboxes multi-select correctly. */
export function dedupeEmailAudienceRecipients(customers = []) {
  ...
  (Array.isArray(customers) ? customers : []).forEach((customer) => {
    ...
    if (!existing || Number(next.totalOrders || 0) > Number(existing.totalOrders || 0)) {
