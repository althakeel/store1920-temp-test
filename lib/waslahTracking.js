...
/** Reverse pickup AWB on the original sale order (waslahReturn). */
export function getReturnEmxTrackingNumber(order = {}) {
  const value = String(order?.waslahReturn?.trackingNumber || '').trim();
  if (looksLikeEmxTrackingNumber(value)) return value;
  if (value && !looksLikeWaslahPlatformTrackingNumber(value) && !/^[a-fA-F0-9]{24}$/.test(value)) {
    return value;
  }
  return '';
}

/** Which AWB the shopper searched — return pickup vs original delivery. */
export function resolveTrackingLookupTarget(order = {}, identifier = '') {
  const raw = String(identifier || '').trim();
  if (!raw) {
    const returnAwb = getReturnEmxTrackingNumber(order);
    const status = String(order?.status || '').toUpperCase();
    if (returnAwb && ['RETURN', 'RETURNED', 'RTO'].includes(status)) {
      return { awb: returnAwb, kind: 'return' };
    }
    const outbound = getEmxTrackingNumber(order);
    return outbound ? { awb: outbound, kind: 'outbound' } : { awb: '', kind: 'outbound' };
  }

  const digits = raw.replace(/\D/g, '');
  const returnCandidates = [
    order?.waslahReturn?.trackingNumber,
    order?.waslahReturn?.reference,
    order?.waslahReturn?.orderId,
  ].map((v) => String(v || '').trim()).filter(Boolean);

  for (const candidate of returnCandidates) {
    if (candidate === raw || (digits && candidate.replace(/\D/g, '') === digits)) {
      return { awb: getReturnEmxTrackingNumber(order) || raw, kind: 'return' };
    }
  }

  return { awb: raw, kind: 'outbound' };
}
...
export function isWaslahCourierOrder(order = {}) {
  ...
  return (
    ...
    || Boolean(order?.waslahReturn?.orderId || order?.waslahReturn?.trackingNumber)
  );
}
