...
import { PUBLIC_STATIC_ROUTES } from '@/lib/sitemapData';
...
const STATIC_LABELS = {
  ...
  '/return-policy': 'Return, Refund, Exchange & Cancellation',
  '/cookie-policy': 'Cookie Policy',
  '/warranty-policy': 'Warranty Policy',
  '/about-us': 'About Us',
  '/business-information': 'Business Information',
  '/create-store': 'Create Your Store',
  '/careers': 'Careers',
  '/pricing': 'Pricing',
...
export async function getHtmlSitemapStoreLinks() {
  ...
  return stores
    .map((store) => {
      ...
      return {
        text: decodeHtmlEntities(store.name || username),
        path: `/shop/${username}`,
...
function accountLinks() {
  ...
  const links = [
    ...
    { text: 'Recommended', path: '/recommended' },
  ];
  if (walletEnabled) {
...
export async function getHtmlSitemapData() {
  const [categoryLinks, blogLinks, storeLinks, productData] = await Promise.all([
    getHtmlSitemapCategoryLinks(),
    getHtmlSitemapBlogLinks(),
    getHtmlSitemapStoreLinks(),
    getHtmlSitemapProductLinks({ page: 1, limit: HTML_SITEMAP_PRODUCT_PAGE_SIZE }),
  ]);
  ...
  const sections = [
    {
      id: 'shop',
      title: 'Shop & Browse',
      links: routesFromSet(SHOP_PATHS),
    },
    {
      id: 'budget',
      title: 'Budget Shopping',
      links: routesFromSet(BUDGET_PATHS),
    },
    {
      id: 'categories',
      title: 'Categories',
    ...
    },
    {
      ...
      title: 'Stores',
      links: storeLinks,
      emptyMessage: 'No public stores yet.',
    },
    {
      id: 'account',
      title: 'Account',
    ...
    },
    {
      id: 'help',
      title: 'Help & Support',
      links: routesFromSet(HELP_PATHS),
    },
    {
      id: 'policies',
      title: 'Policies & Legal',
      links: routesFromSet(POLICY_PATHS),
    },
    {
      id: 'about',
      title: 'About & Partnership',
      links: routesFromSet(ABOUT_PATHS),
    },
  ];
