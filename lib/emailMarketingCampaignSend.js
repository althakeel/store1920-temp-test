...
export async function loadCampaignProducts(campaign = {}) {
  let blocks = Array.isArray(campaign.blocks) ? campaign.blocks : [];
  if (!blocks.length && campaign.customTemplateId) {
    const saved = await EmailTemplate.findById(campaign.customTemplateId)
      .select('blocks')
      .lean();
  ...
  const selectedIds = collectCampaignProductIds({ ...campaign, blocks });
  const query = {
    inStock: true,
    stockQuantity: { $gt: 0 },
  };
  if (selectedIds.length) {
    ...
    const chosen = await Product.find({ _id: { $in: selectedIds } })
      .select('_id name slug price mrp AED images description shortDescription category categories categoryName brand sku stockQuantity createdAt')
      .lean();
...
/** Same layout for every recipient; only unsubscribe email is swapped later. */
export function renderSharedCampaignHtml(content) {
  return absolutizeEmailHtmlImages(content.render(''));
}

export function personalizeCampaignHtml(html = '', recipientEmail = '') {
  const email = encodeURIComponent(String(recipientEmail || '').trim());
  if (!email) return html;
  return String(html || '').replace(
    /unsubscribe\?unsubscribe=promotional(?:&(?:amp;)?email=[^"'&\s]*)?/gi,
    `unsubscribe?unsubscribe=promotional&email=${email}`,
  );
}
...
export async function sendEmailMarketingCampaign(campaign, { slotKey = '' } = {}) {
  ...
  const sharedHtml = renderSharedCampaignHtml(content);
