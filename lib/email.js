...
import { buildCustomerSitePath, getCustomerSiteUrl } from '@/lib/appUrl';
...
export async function sendReturnRequestCustomerEmail({
  email,
  name,
  order,
  returnNumber = '',
  ...
  const label = returnNumber ? `${returnNumber}` : orderNoLabel(order);
  return sendStyledStatusEmail({
    email,
    name,
    order,
    subject: `${title} - ${label}`,
    title,
    intro: intro || 'There is an update on your return or replacement request.',
  });
}
...
export async function sendMail({
  ...
  const isMarketingMail = fromType === 'marketing' || fromType === 'promotional';
  const {
    html: emailHtml,
    ...
    // Marketing: pass HTML through exactly (no logo bar, no transactional layout wrapper).
    injectMissingLogo: !isMarketingMail,
  });
      ...
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="benefits">
            ...
            <div class="benefit-item">
              <span class="benefit-icon">✓</span>
              <span>Easy returns per our return policy</span>
            </div>
          </div>

          <div class="button-container">
            <a href="${getCustomerSiteUrl()}" class="button">Start Shopping</a>
          </div>
...
export async function sendOrderShippedEmail(orderData) {
  ...
  const bodyHtml = [
    ...
    storeTrackUrl
      ...
      : renderEmailCta({
        label: 'Track Order',
        href: `${getCustomerSiteUrl()}/track-order`,
        align: 'center',
      }),
  ].filter(Boolean).join('');
...
export async function sendGuestAccountCreationEmail(guestData) {
  ...
  const signInUrl = `${getCustomerSiteUrl()}/sign-in`;
...
export async function sendPaymentCancelledRecoveryEmail({
  ...
  const checkoutLink = checkoutUrl || `${getCustomerSiteUrl()}/checkout`;
...
export async function sendAdminPickupOverdueEmail({ orders = [], cutoff } = {}) {
  const adminEmails = getAdminOrderNotificationEmails();
  if (!adminEmails.length) {
    console.warn('[email] No ADMIN_EMAIL configured — skipping pickup overdue alert');
    return { skipped: true, reason: 'no_admin_email' };
  }
  ...
  const pickupUrl = buildCustomerSitePath('/store/todays-pickup');
  const rows = list.slice(0, 40).map((order) => {
    const orderNo = escapeHtml(getDisplayOrderNumber(order) || String(order._id || '').slice(-6));
    const requestedAt = order?.waslah?.pickupRequestedAt
      ? escapeHtml(formatEmailDate(order.waslah.pickupRequestedAt))
      : '—';
    const pickupSlot = [
      order?.waslah?.pickupDate,
      order?.waslah?.pickupTime,
    ].filter(Boolean).map(escapeHtml).join(' · ') || '—';
    const status = escapeHtml(String(order.status || '').replace(/_/g, ' '));
    const hours = order?.waslah?.pickupRequestedAt
      ? ((Date.now() - new Date(order.waslah.pickupRequestedAt).getTime()) / (60 * 60 * 1000)).toFixed(1)
      : '—';

    return `<tr>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#111;">#${orderNo}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#111;">${status}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#111;">${requestedAt}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#111;">${pickupSlot}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#b45309;text-align:right;">${hours}h</td>
    </tr>`;
  }).join('');

  const bodyHtml = [
    `<p style="margin:0 0 16px;font-size:14px;line-height:1.7;color:#444444;">
      ${list.length} order(s) still have <strong>pickup requested</strong> and were not picked up within 24 hours.
      Cutoff: ${escapeHtml(cutoff ? formatEmailDate(cutoff) : '24 hours ago')}.
    </p>`,
    `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;border:1px solid #e8e8e8;border-collapse:collapse;">
      <tr style="background:#fff7ed;">
        ...
        <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Status</th>
        <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Requested</th>
        <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Pickup slot</th>
        <th align="right" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Waiting</th>
      </tr>
      ${rows}
    </table>`,
    list.length > 40
      ? `<p style="margin:0 0 16px;font-size:13px;color:#666;">Showing first 40 of ${list.length} overdue pickups.</p>`
      : '',
    renderEmailCta({
      label: 'Open Today\'s Pickup list',
      href: pickupUrl,
      align: 'center',
    }),
  ].filter(Boolean).join('');
  ...
  const html = await buildTransactionalEmail({
    title: 'Pickup overdue (24h+)',
    greeting: 'Team',
    intro: 'Courier pickup was requested but these parcels are still waiting at the warehouse.',
    bodyHtml,
  });

  const [primaryAdmin, ...otherAdmins] = adminEmails;
  const subject = `Pickup overdue · ${list.length} order(s) waiting 24h+`;
  ...
  return { sent: true, to: adminEmails, subject, count: list.length };
}
