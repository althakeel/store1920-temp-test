...
function isPopulatedProductDoc(value) {
  return Boolean(
    value
    && typeof value === 'object'
    && !Array.isArray(value)
    && value._bsontype !== 'ObjectId'
    && (value.name || value.title || value.slug || value.image || Array.isArray(value.images)),
  );
}
...
export function getOrderLineProduct(item) {
  if (!item) return {};
  if (isPopulatedProductDoc(item.product)) return item.product;
  if (isPopulatedProductDoc(item.productId)) return item.productId;
  return {};
}
