...
export const OFFERS_NAV_LABEL_MAX = 60;
export const OFFERS_NAV_EMOJI_PRESETS = ['🔥', '⚡', '💥', '🎁', '🏷️', '⏰', '✨', '🚀', '❤️', '🎉'];
export const OFFERS_NAV_EMOJI_FONT_FALLBACK =
  '"Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji"';

export function getOffersNavEmojiImageSrc(emoji = '') {
  const codes = [];
  for (const char of String(emoji || '')) {
    const codePoint = char.codePointAt(0);
    if (!codePoint || codePoint === 0xfe0f || codePoint === 0xfe0e) continue;
    codes.push(codePoint.toString(16));
  }
  if (!codes.length) return '';
  return `https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72/${codes.join('-')}.png`;
}
export const OFFERS_NAV_SHORTCODES = {
  fire: '🔥',
  flame: '🔥',
  zap: '⚡',
  lightning: '⚡',
...
export function expandOffersNavShortcodes(value = '') {
  return String(value).replace(/[:：]\s*([a-z0-9_+-]+)\s*[:：]/gi, (match, name) => (
    OFFERS_NAV_SHORTCODES[String(name || '').toLowerCase()] || match
  ));
}
...
function withEmojiPresentation(value = '') {
  ...
  return `${raw.replace(/\uFE0E/g, '')}\uFE0F`;
}

export function splitOffersNavTextParts(label = '') {
  const text = expandOffersNavShortcodes(label);
  if (!text) return [];
  const parts = [];
  let regex;
  try {
    regex = /\p{Extended_Pictographic}(?:\uFE0F|\uFE0E)?(?:\u200D\p{Extended_Pictographic}(?:\uFE0F|\uFE0E)?)*/gu;
  } catch {
    regex = /[\u{1F300}-\u{1FAFF}](?:\uFE0F|\uFE0E)?/gu;
  }
  let lastIndex = 0;
  ...
  while (match) {
    ...
    parts.push({ type: 'emoji', value: withEmojiPresentation(match[0]) });
...
function normalizeNavLabel(value, fallback = '') {
  return expandOffersNavShortcodes(String(value ?? fallback)).trim().slice(0, OFFERS_NAV_LABEL_MAX);
}

export function normalizeNavGifUrl(value = '') {
...
export function offersNavLabelHasEmoji(label = '') {
  try {
    return /\p{Extended_Pictographic}/u.test(String(label || ''));
  } catch {
    return /[\u{1F300}-\u{1FAFF}]/u.test(String(label || ''));
  }
}

export function withOffersNavEmojiFonts(fontFamily = '') {
  const base = String(fontFamily || '').trim();
  if (!base || base === 'inherit') {
    return `system-ui, -apple-system, BlinkMacSystemFont, ${OFFERS_NAV_EMOJI_FONT_FALLBACK}, sans-serif`;
  }
  if (base.includes('Apple Color Emoji')) return base;
  return `${base}, ${OFFERS_NAV_EMOJI_FONT_FALLBACK}`;
}
...
export function getOffersNavButtonAppearance(style = {}, variant = 'desktop') {
  ...
  return {
    ...
    style: {
      fontFamily: withOffersNavEmojiFonts(fontOption.value),
...
export function normalizeOffersPage(payload = {}) {
  ...
  return {
    ...
    navGifUrl: normalizeNavGifUrl(src.navGifUrl),
...
export function getOffersPageCopy(settings = {}, language = 'en') {
  ...
  return {
    ...
    navGifUrl: normalized.navGifUrl,
