...
export function normalizeOfferProduct(product, language = 'en') {
  const localized = localizeRecord(product, language, ['name', 'brand']);
  const discount = Math.round(Number(product?.__discountPct || 0));
  const images = Array.isArray(localized.images) ? localized.images.slice(0, 2) : [];
  ...
  return {
    ...
    categories: Array.isArray(localized.categories) ? localized.categories : [],
    inStock: localized.inStock !== false,
    stockQuantity: localized.stockQuantity,
    imageAspectRatio: localized.imageAspectRatio || '1:1',
    useProductsPath: Boolean(localized.useProductsPath),
    cardVideoPreviewEnabled: Boolean(localized.cardVideoPreviewEnabled),
    cardVideoPreviewDelaySec: localized.cardVideoPreviewDelaySec,
    fastDelivery: Boolean(localized.fastDelivery),
    freeShippingEligible: Boolean(localized.freeShippingEligible),
    legacySourceId: localized.legacySourceId || null,
    discount: discount > 0 ? discount : null,
    averageRating: localized.averageRating,
    ratingCount: localized.ratingCount,
  };
}
