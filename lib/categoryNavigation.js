...
function localizeNavLink(link = {}, categories = [], language = 'en') {
  const match = findCategoryForNavItem(link, categories, language);
  if (!match) return link;
  const localized = getCategoryDisplayName(match, language);
  return {
    ...link,
    name: localized,
    title: localized,
    slug: link?.slug || match?.slug || '',
    categoryId: link?.categoryId || getCategoryRecordId(match),
  };
}

function enrichExistingNavLinks(links = [], categories = [], language = 'en') {
  return (Array.isArray(links) ? links : []).map((link) => {
    const localized = localizeNavLink(link, categories, language);
    const existingChildren = Array.isArray(localized?.children) ? localized.children.filter(Boolean) : [];
    if (existingChildren.length) {
      return { ...localized, children: enrichExistingNavLinks(existingChildren, categories, language) };
    }

    const childMatch = findCategoryForNavItem(localized, categories, language);
    if (!childMatch) return localized;
...
export function enrichNavItemWithCategoryChildren(item = {}, categories = [], language = 'en') {
  ...
  const localizedItem = localizeNavLink(item, catalog, language);
  const existing = Array.isArray(localizedItem?.megaMenu?.links) ? localizedItem.megaMenu.links.filter(Boolean) : [];
  if (existing.length) {
    return {
      ...localizedItem,
      hasDropdown: true,
      megaMenu: {
        ...(localizedItem.megaMenu && typeof localizedItem.megaMenu === 'object' ? localizedItem.megaMenu : {}),
        links: enrichExistingNavLinks(existing, catalog, language),
      },
    };
  }

  const match = findCategoryForNavItem(localizedItem, catalog, language);
  if (!match) return localizedItem;

  const links = mapChildNavLinks(catalog, match, language);
  if (!links.length) return localizedItem;
  ...
  return {
    ...
    megaMenu: {
      ...
      links,
      images: Array.isArray(localizedItem?.megaMenu?.images) ? localizedItem.megaMenu.images : [],
    },
  };
}
