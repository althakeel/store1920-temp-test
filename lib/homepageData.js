...
async function fetchFeaturedProducts(limit = 12, language = 'en') {
  const cacheKey = `public:featured-products:v1:${limit}:${language}`;
  const cached = getCachedData(cacheKey);
  if (cached) return cached;
  ...
  const payload = {
    products,
    sectionTitle: isArabic && sectionTitleAr ? sectionTitleAr : sectionTitleEn,
    sectionDescription: isArabic && sectionDescriptionAr ? sectionDescriptionAr : sectionDescriptionEn,
    sectionTitleEn,
