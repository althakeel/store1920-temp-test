...
function buildWaslahReturnLinkFields(rawOrder = {}, {
  reference = '',
  returnRequestNumber = '',
} = {}) {
  const originalTracking = String(
    rawOrder.waslah?.emxTrackingNumber
    || rawOrder.waslah?.trackingNumber
    || rawOrder.trackingId
    || '',
  ).trim();
  return {
    'waslahReturn.originalTrackingNumber': originalTracking || null,
    'waslahReturn.originalWaslahOrderId': String(rawOrder.waslah?.orderId || '').trim() || null,
    'waslahReturn.returnRequestNumber': String(returnRequestNumber || '').trim() || null,
    'waslahReturn.reference': reference || null,
  };
}
...
export async function shipReversePickupForOriginalOrder({
  ...
  const linkFields = buildWaslahReturnLinkFields(rawOrder, {
    reference,
    returnRequestNumber,
  });

  // Refresh return AWB from Waslah when reverse pickup already exists.
  if (rawOrder.waslahReturn?.orderId) {
    try {
      const { syncWaslahReturnTrackingForOrder } = await import('@/lib/waslahOrderStatusSync');
      const synced = await syncWaslahReturnTrackingForOrder(rawOrder, { persist: true });
      const syncedOrder = synced?.order || rawOrder;
      const syncedTracking = String(syncedOrder?.waslahReturn?.trackingNumber || '').trim();
      if (!String(syncedOrder?.waslahReturn?.originalTrackingNumber || '').trim() && linkFields['waslahReturn.originalTrackingNumber']) {
        await Order.findByIdAndUpdate(normalizedOrderId, { $set: linkFields });
      }
      if (syncedTracking) {
        return {
          success: true,
          ...
          waslahOrderId: syncedOrder.waslahReturn?.orderId || rawOrder.waslahReturn.orderId,
          trackingNumber: syncedTracking,
          originalTrackingNumber: syncedOrder.waslahReturn?.originalTrackingNumber
            || linkFields['waslahReturn.originalTrackingNumber'],
          message: synced.changed
            ? `Return pickup tracking updated to ${syncedTracking}.`
            : 'Reverse pickup is already on EMX for this order.',
        };
      }
    } catch (syncError) {
      console.warn('[waslah-reverse-pickup] Existing return refresh failed:', syncError?.message || syncError);
    }
  }
  ...
  const originalTrackingNumber = linkFields['waslahReturn.originalTrackingNumber'] || null;
