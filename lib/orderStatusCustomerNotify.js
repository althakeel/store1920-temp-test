...
export async function notifyCustomerOfOrderStatusChange(order, status, {
  ...
  const latest = await Order.findById(order._id)
    .select('status statusEmailSentAt guestEmail guestName shippingAddress userId isGuest shortOrderNumber trackingId trackingUrl courier warehousePacking waslah')
    .lean();
  const alreadySent = latest?.statusEmailSentAt?.[next];
  ...
  try {
    ...
    const failed = emailResult?.sent === false || emailResult?.reason === 'no_email';
    ...
    if (!failed && (next === 'SHIPPED' || next === 'DELIVERED')) {
      try {
        ...
        await appendOrderCommunicationLog(order._id, {
          ...
          sentByUid: actorUid,
          sentByName: actorName,
          details: whatsappResult?.success
            ? ''
    ...
    return {
      sent: !failed,
      reason: failed ? (emailResult?.reason || 'not_sent') : 'sent',
