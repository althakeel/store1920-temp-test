...
/** Read seller/import stock from the fields we actually save. */
export function getVariantStock(variant) {
  if (!variant || typeof variant !== 'object') return 0;
  const primary = Number(variant.stock);
  if (Number.isFinite(primary)) return primary;
  const fallbacks = [variant.stockQuantity, variant.quantity, variant.inventory, variant.qty];
...
export function getProductBundleMode(product = {}) {
  ...
  if (variantType === 'variant_bundles') {
    const hasPackRow = variants.some((variant) => Number(variant?.options?.bundleQty) > 0);
    return hasPackRow ? 'matrix' : 'none';
  }
...
export function buildVariantOptionGroups(variants = [], { isBulkBundleVariant = isBulkBundleVariantOption } = {}) {
  ...
  return dedupeVariantOptionGroups(
    sortedKeys
      .map((key) => ({
        key,
...
function optionValuesAlign(sourceValues = [], targetValues = []) {
  if (!sourceValues.length || sourceValues.length !== targetValues.length) return false;
  const used = new Set();
  return sourceValues.every((source) => {
    const needle = normalizeOptionValue(source);
    if (!needle) return false;
    const matchIndex = targetValues.findIndex((target, index) => {
      if (used.has(index)) return false;
      const haystack = normalizeOptionValue(target);
...
/** Drop a second picker when title/color (etc.) are the same choice with different labels. */
export function dedupeVariantOptionGroups(groups = []) {
  const list = (Array.isArray(groups) ? groups : []).filter((group) => group?.values?.length);
  if (list.length < 2) return list;

  const drop = new Set();
  const preferKeep = new Set(['color', 'size']);

  for (let i = 0; i < list.length; i += 1) {
    for (let j = i + 1; j < list.length; j += 1) {
      const a = list[i];
      const b = list[j];
      if (!optionValuesAlign(a.values, b.values) && !optionValuesAlign(b.values, a.values)) {
        continue;
      }

      const aPreferred = preferKeep.has(a.key);
      const bPreferred = preferKeep.has(b.key);
      if (aPreferred && !bPreferred) drop.add(b.key);
      else if (bPreferred && !aPreferred) drop.add(a.key);
      else {
        const aChars = a.values.join('').length;
        const bChars = b.values.join('').length;
        drop.add(aChars >= bChars ? a.key : b.key);
      }
    }
  }

  return list.filter((group) => !drop.has(group.key));
}
...
/** Keep only picker-visible keys so a hidden duplicate (title vs color) cannot block stock. */
export function limitSelectedOptionsToGroups(selectedOptions = {}, groups = []) {
  const allowed = new Set((Array.isArray(groups) ? groups : []).map((group) => group?.key).filter(Boolean));
  const next = {};
...
export function formatVariantOptionValue(value = '') {
  const raw = String(value || '').trim();
  if (!raw) return '';
  let next = raw.replace(/^\d{2}[- ]/, '');
  const parts = next.split('-').filter(Boolean);
  if (parts.length >= 4) {
    next = parts.slice(-2).join('-').replace(/^\d{2}-/, '');
  }
  return next.replace(/[-_]+/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase());
}
...
export function findVariantForOptionValue(
  ...
  groupKey,
  value,
  { isBulkBundleVariant = isBulkBundleVariantOption, ignoreOtherSelections = false } = {},
) {
  return variants.find((variant) => {
    if (isBulkBundleVariant(variant)) return false;
    const options = normalizeSellerVariantOptions(variant.options || {});
    if (String(options?.[groupKey] || '') !== String(value)) return false;
    if (ignoreOtherSelections) return true;
...
export function isVariantOptionValueAvailable(
  ...
  return variants.some((variant) => {
    ...
    return matchesOtherSelections && getVariantStock(variant) > 0;
  });
}
...
export function getVariantOptionImage(variant, productImages = [], { fallbackToProduct = true } = {}) {
  ...
  if (!fallbackToProduct) return null;
