...
import app, { waitForAuthReady, ensureLocalAuthPersistence } from './firebase';
...
function signInWithSocialPopup(provider) {
  return signInWithPopup(clientAuth(), provider, browserPopupRedirectResolver);
}
...
export async function signInWithGooglePopup() {
  return runWithAuthLock(() =>
    withTimeout(
      signInWithSocialPopup(createGoogleProvider()),
      POPUP_TIMEOUT_MS,
...
export async function signInWithFacebookPopup() {
  return runWithAuthLock(() =>
    withTimeout(
      signInWithSocialPopup(createFacebookProvider()),
      POPUP_TIMEOUT_MS,
...
export async function signInWithApplePopup() {
  return runWithAuthLock(() =>
    withTimeout(
      signInWithSocialPopup(createAppleProvider()),
      POPUP_TIMEOUT_MS,
...
export async function signInWithGoogleRedirect() {
  return runWithAuthLock(() =>
    signInWithRedirect(clientAuth(), createGoogleProvider(), browserPopupRedirectResolver)
  );
}

export async function consumeGoogleRedirectResult() {
  await waitForAuthReady();
  return getRedirectResult(clientAuth(), browserPopupRedirectResolver);
}

export async function signInWithGoogleIdToken(idToken) {
  const token = String(idToken || '').trim();
  if (!token) {
    throw Object.assign(new Error('Missing Google ID token'), { code: 'auth/invalid-credential' });
  }
  const firebaseCredential = GoogleAuthProvider.credential(token);
  return runWithAuthLock(() => signInWithCredential(clientAuth(), firebaseCredential));
}

/** @deprecated Prefer signInWithGoogleIdToken(idToken) */
export async function signInWithGoogleCredential(credential) {
  return signInWithGoogleIdToken(credential);
}

export async function signInWithEmail(email, password) {
  return runWithAuthLock(() => signInWithEmailAndPassword(clientAuth(), email, password));
}
