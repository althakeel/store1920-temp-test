...
/** Checkout, address, and payment APIs — no rate limit for now. */
export function isCheckoutPaymentApiPath(pathname = '') {
  ...
  return (
    ...
    || path === '/api/abandoned-checkout'
    || path.startsWith('/api/abandoned-checkout/')
  );
}
...
export function resolveApiRateLimit(pathname = '', method = 'GET') {
  ...
  if (isCheckoutPaymentApiPath(path)) {
    return { max: 2000, windowMs: 60_000, tier: 'checkout' };
  }
...
export function resolveBurstLimit(pathname = '', method = 'GET') {
  ...
  if (isPublicReadApiPath(pathname, method)) {
    return { max: 800, windowMs: 10_000, tier: 'burst-public' };
  }
  return { max: 400, windowMs: 10_000, tier: 'burst' };
}
