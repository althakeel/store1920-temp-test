...
export function buildProductJsonLd(product = {}, { categoryChain = [], siteUrl = '' } = {}) {
  ...
  const seoDescription = stripHtml(product.seoDescription || '');
  const longDescription = stripHtml(product.description || '');
  const shortDescription = stripHtml(product.shortDescription || '');
  const description = (
    seoDescription
    || (longDescription.length >= shortDescription.length ? longDescription : shortDescription)
    || name
  ).slice(0, 5000);
...
export function buildProductSeoJsonLdDocuments({
  ...
  const productDoc = buildProductJsonLd(product, { categoryChain, siteUrl });
  const reviewDoc = buildProductReviewJsonLd(product, reviews, siteUrl);
  if (reviewDoc?.aggregateRating) {
    productDoc.aggregateRating = reviewDoc.aggregateRating;
  }
  if (reviewDoc?.review) {
    productDoc.review = reviewDoc.review;
  }
