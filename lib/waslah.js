...
export async function createOrLinkWaslahOrder(payload, { fallbackReference } = {}) {
  ...
  duplicateError.hint = 'Open ship.waslah.ae, cancel shipment S1920-618681 if it is still active, then ship again from Store1920.';
...
export async function cancelWaslahOrder(waslahOrderId) {
  const id = String(waslahOrderId || '').trim();
  if (!id) {
  ...
  const attempts = [
    ...
    { method: 'PUT', path: `/orders/${id}`, body: { deleted: true } },
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (error?.status && ![404, 405, 400, 422].includes(error.status)) {
        throw error;
      }
    }
  }
  ...
  try {
    ...
    if (error?.status === 404) {
      return accepted || { ok: true, alreadyCancelled: true, path: '/orders/:id', data: null };
    }
  }
