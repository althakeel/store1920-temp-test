...
export const WABA_TEMPLATE_NAMES = {
  ...
  otp: String(process.env.WABA_TEMPLATE_OTP || 'store1920_otp').trim(),
};
...
/**
 * Meta URL buttons append this value onto the approved template URL.
 * store1920_order_delivered Shop Again is `https://www.store1920.com/product/{{1}}`,
 * so {{1}} must be the product slug only — not `/product/slug` or `/products`.
 */
export function toWhatsAppProductUrlSuffix(productOrPath = '') {
  const raw = typeof productOrPath === 'object' && productOrPath
    ? String(productOrPath.slug || productOrPath.id || productOrPath._id || '').trim()
    : String(productOrPath || '').trim();
  if (!raw) return '';
  ...
  if ((parts[0] === 'product' || parts[0] === 'products') && parts.length > 1) {
