...
import { authorizeMetaPurchase, installMetaPurchaseGuard, markOrderPurchaseIdentifiersSent } from '@/lib/metaPurchaseGuard';
...
function hasPagePurchaseLock() {
  return typeof window !== 'undefined' && Boolean(window[PAGE_PURCHASE_LOCK_KEY]);
}

function setPagePurchaseLock() {
...
/** True only after fbevents.js is loaded — not just the fbq stub. */
export function isFbqFullyLoaded() {
  if (typeof window === 'undefined' || !window.fbq) return false;
  const rooted = window.fbq.__purchaseGuardOriginal || window.fbq;
  return typeof rooted.__realCallMethod === 'function';
}

function isFbqReadyForEvent(eventName) {
  if (!getBareFbq()) return false;
  if (eventName === 'Purchase') return isFbqFullyLoaded();
  return true;
}

export function waitForFbqFullyLoaded(maxMs = 15000) {
...
function invokeBareFbq(...args) {
  ...
  try {
    if (isPurchaseFbqArgs(args) && !isFbqFullyLoaded()) {
      return false;
    }
...
function sendPurchaseViaBareFbq(payload, options = {}) {
  ...
  if (hasPagePurchaseLock()) return false;

  // Lock BEFORE fbq so GTM / parallel callers cannot send a 2nd Purchase.
  setPagePurchaseLock();
  authorizeMetaPurchase(options.eventID);
  markOrderPurchaseIdentifiersSent({
    _id: options.eventID,
    id: options.eventID,
    shortOrderNumber: payload?.transaction_id,
    orderNumber: payload?.transaction_id,
  });

  return invokeBareFbq(
    'trackSingle',
    META_PIXEL_ID,
    'Purchase',
    payload,
...
function sendMetaEventNow(eventName, params = {}, options = {}) {
  ...
  if (!isFbqReadyForEvent(eventName)) return false;

  const isPurchase = eventName === 'Purchase' && options?.eventID;
  if (isPurchase && hasPagePurchaseLock()) {
    return false;
  }
...
function scheduleFbqDrain() {
  ...
  pending.forEach(({ eventName, params, options }) => {
        // Defensive: never drain Purchase from queue (Purchase is send-now-only).
        if (eventName === 'Purchase') {
          const dedupeKey = resolveMetaDedupeKey(eventName, options);
          if (dedupeKey) FBQ_QUEUED_KEYS.delete(dedupeKey);
          return;
        }
        const dedupeKey = resolveMetaDedupeKey(eventName, options);
        try {
...
export const trackMetaEvent = (eventName, params = {}, options = {}) => {
  ...
  const dedupeKey = resolveMetaDedupeKey(eventName, options);
  if (dedupeMode === 'burst') {
    if (!isFbqReadyForEvent(eventName) && dedupeKey && FBQ_QUEUED_KEYS.has(dedupeKey)) {
      return false;
    }
    if (!claimBurstDedupe(dedupeKey || `meta:${eventName}:burst`, options.burstMs || 600)) {
      return false;
  ...
  // Purchase must NEVER queue on the fbq stub — stub replay caused double Purchase.
  // order-success retries until fbevents.js is fully loaded.
  if (!isFbqReadyForEvent(eventName)) {
    if (isPurchase) {
      return false;
    }
    if (dedupeKey) {
