...
async function persistLinkedOrder(order, doc) {
  ...
  if (order?._id) {
    ...
    if (typeof order.isModified === 'function' ? order.isModified('status') : false) {
...
async function completeQualityCheck(doc, { order, actor = {}, passed, qc = {} } = {}) {
  ...
  try {
    ...
    await restockOrderInventory(restockSource);
  } catch (error) {
...
export async function syncReturnPickupFromFollowUpOrder(order) {
  ...
  const riderHint = String(order.waslah?.lastLocation || '').trim();
  ...
  pushHistory(doc, mapped, order.waslah?.currentStatus || orderStatus, { name: 'Courier' });
