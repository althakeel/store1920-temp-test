...
const SENT_PURCHASE_IDS_KEY = '__metaPurchaseSentIds';
const PAGE_PURCHASE_LOCK_KEY = '__store1920MetaPurchaseHandled';
...
function getSentPurchaseIds() {
  if (typeof window === 'undefined') return null;
  const set = window[SENT_PURCHASE_IDS_KEY];
  return set && typeof set.has === 'function' ? set : null;
}

function markPurchaseSent(eventId) {
  if (!eventId || typeof window === 'undefined') return;
  window[SENT_PURCHASE_IDS_KEY] = window[SENT_PURCHASE_IDS_KEY] || new Set();
  window[SENT_PURCHASE_IDS_KEY].add(String(eventId).trim());
}

function collectPurchaseIds(args = []) {
  const ids = new Set();
  const eventName = resolveGatedEventName(args);
  if (eventName !== 'Purchase') return ids;
  ...
  const cmd = args[0];
  let payload = null;
  if (cmd === 'trackSingle' && args[2] === 'Purchase') payload = args[3];
  else if (cmd === 'track' && args[1] === 'Purchase') payload = args[2];

  if (payload && typeof payload === 'object') {
    ['order_id', 'transaction_id', 'event_id', 'eventID'].forEach((key) => {
...
function wasAnyPurchaseSent(args = []) {
  const sent = getSentPurchaseIds();
  if (!sent || sent.size === 0) return false;
  for (const id of collectPurchaseIds(args)) {
    if (sent.has(id)) return true;
  }
  return false;
}

function markAllPurchaseSent(args = []) {
  collectPurchaseIds(args).forEach((id) => markPurchaseSent(id));
}

/** Pre-mark every id GTM might use so ga4_purchase cannot fire a 2nd Meta Purchase. */
export function markOrderPurchaseIdentifiersSent(order = {}) {
  const mongoId = String(order._id || order.id || '').trim();
  const shortNumber = String(order.shortOrderNumber ?? order.orderNumber ?? '').trim();
  markPurchaseSent(mongoId);
  if (shortNumber) markPurchaseSent(shortNumber);
}

function readEventIdFromObject(value) {
  if (!value || typeof value !== 'object') return '';
  return String(
    value.eventID || value.event_id || value.order_id || value.transaction_id || ''
  ).trim();
}
...
function shouldAllowGatedCall(args) {
  ...
  if (eventName === 'Purchase') {
    if (typeof window !== 'undefined' && window[PAGE_PURCHASE_LOCK_KEY]) {
      return false;
    }
    // Block every duplicate Purchase on this pixel (app + GTM), even if re-authorized.
    if (wasAnyPurchaseSent(args)) return false;
...
function runGuardedFbqCall(original, args) {
  ...
  if (isGatedFbqCall(args) && result !== false) {
    const eventName = resolveGatedEventName(args);
    if (eventName === 'Purchase') {
      markAllPurchaseSent(args);
      if (typeof window !== 'undefined') {
        window[PAGE_PURCHASE_LOCK_KEY] = true;
      }
    } else {
      const eventId = resolveGatedEventId(args);
...
var k='${AUTHORIZED_IDS_KEY}',s='${SENT_PURCHASE_IDS_KEY}',p='${PAGE_PURCHASE_LOCK_KEY}',g='${GUARD_FLAG}';
...
window[s]=window[s]||new Set();
function readId(o){
  if(!o||typeof o!=='object')return '';
  return String(o.eventID||o.event_id||o.order_id||o.transaction_id||'').trim();
}
function collectPurchaseIds(args){
  var ids=[],seen={},name=gatedName(args),i,id;
  if(name!=='Purchase')return ids;
  id=gatedEventId(args);
  if(id&&!seen[id]){seen[id]=1;ids.push(id);}
  var payload=null;
  if(args[0]==='trackSingle'&&args[2]==='Purchase')payload=args[3];
  else if(args[0]==='track'&&args[1]==='Purchase')payload=args[2];
  if(payload&&typeof payload==='object'){
    ['order_id','transaction_id','event_id','eventID'].forEach(function(key){
      id=String(payload[key]||'').trim();
      if(id&&!seen[id]){seen[id]=1;ids.push(id);}
    });
  }
  return ids;
...
function wasAnyPurchaseSent(args){
  if(!window[s]||!window[s].size)return false;
  var ids=collectPurchaseIds(args),i;
  for(i=0;i<ids.length;i++){if(window[s].has(ids[i]))return true;}
  return false;
}
function markAllPurchaseSent(args){
  var ids=collectPurchaseIds(args),i;
  for(i=0;i<ids.length;i++){window[s].add(ids[i]);}
}
...
function allow(args){
  ...
  if(gatedName(args)==='Purchase'){
    if(window[p])return false;
    if(wasAnyPurchaseSent(args))return false;
  }
