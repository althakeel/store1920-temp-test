...
function wasPurchaseSent(eventId) {
  const id = String(eventId || '').trim();
  if (!id) return false;
  return Boolean(getSentPurchaseIds()?.has(id));
}
...
function shouldAllowGatedCall(args) {
  ...
  const eventName = resolveGatedEventName(args);
  const eventId = resolveGatedEventId(args);
  if (eventName === 'Purchase' && eventId && wasPurchaseSent(eventId)) {
    return false;
  }
...
function runGuardedFbqCall(original, args) {
  ...
  if (isGatedFbqCall(args) && result !== false) {
    ...
    if (eventName === 'Purchase' && eventId) {
      ...
      getAuthorizedEventIds()?.delete(eventId);
    }
  }
  return result;
}
...
var k='${AUTHORIZED_IDS_KEY}',s='${SENT_PURCHASE_IDS_KEY}',g='${GUARD_FLAG}';
...
function allow(args){
  ...
  if(name==='Purchase'&&eid&&window[s]&&window[s].has(eid))return false;
...
function wrap(fbq){
  ...
  function invoke(args){
    ...
    if(isGated(args)&&result!==false){
      var name=gatedName(args);
      var eid=gatedEventId(args);
      if(name==='Purchase'&&eid&&window[s])window[s].add(eid);
      else if(eid&&window[k])window[k].delete(eid);
    }
    return result;
  }
