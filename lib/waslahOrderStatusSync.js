...
export function canSyncWaslahOrderStatus(order = {}) {
  ...
  // Return reverse pickups live under waslahReturn (separate from outbound waslah).
  return Boolean(
    trackingId
...
export function shouldSyncWaslahOrderStatus(order = {}) {
  ...
  // Still refresh return pickup AWB even when outbound shipment is terminal.
  if (String(order?.waslahReturn?.orderId || '').trim()) {
    const returnTracking = String(order?.waslahReturn?.trackingNumber || '').trim();
    if (!returnTracking || looksLikeWaslahPlatformTrackingNumber(returnTracking) || !looksLikeEmxTrackingNumber(returnTracking)) {
      return true;
    }
  }
...
/**
 * Recover / update EMX AWB on the reverse pickup (waslahReturn), when Waslah
 * later assigns a different tracking number than what we stored at create time.
 */
export async function syncWaslahReturnTrackingForOrder(order = {}, { persist = true } = {}) {
  const returnOrderId = String(order?.waslahReturn?.orderId || '').trim();
  if (!returnOrderId || !isWaslahConfigured()) {
    return { order, changed: false, skipped: true };
  }

  try {
    const { getWaslahOrder, extractWaslahShipmentDetails } = await import('@/lib/waslah');
    const detail = await getWaslahOrder(returnOrderId);
    const details = extractWaslahShipmentDetails({
      waslahOrder: detail,
      waslahOrderId: returnOrderId,
    });

    const recoveredEmx = String(details.emxTrackingNumber || '').trim();
    const recoveredAny = String(details.trackingNumber || details.emxTrackingNumber || '').trim();
    const prevTracking = String(order.waslahReturn?.trackingNumber || '').trim();

    let nextTracking = prevTracking;
    if (looksLikeEmxTrackingNumber(recoveredEmx)) {
      nextTracking = recoveredEmx;
    } else if (recoveredAny && (!prevTracking || looksLikeWaslahPlatformTrackingNumber(prevTracking))) {
      nextTracking = recoveredAny;
    } else if (recoveredAny && recoveredAny !== prevTracking && looksLikeEmxTrackingNumber(recoveredAny)) {
      nextTracking = recoveredAny;
    }

    const nextLabel = String(details.labelUrl || '').trim();
    const trackingChanged = Boolean(nextTracking && nextTracking !== prevTracking);
    const labelChanged = Boolean(nextLabel && nextLabel !== String(order.waslahReturn?.labelUrl || '').trim());
    const syncAt = new Date();

    const waslahReturn = {
      ...(order.waslahReturn || {}),
      orderId: returnOrderId,
      trackingNumber: nextTracking || prevTracking || null,
      processed: Boolean(nextTracking || prevTracking),
      lastStatusSyncAt: syncAt,
      ...(nextLabel ? { labelUrl: nextLabel } : {}),
    };

    const linkBackfill = {};
    const originalTracking = String(
      order.waslah?.emxTrackingNumber
      || order.waslah?.trackingNumber
      || order.trackingId
      || '',
    ).trim();
    if (!String(order.waslahReturn?.originalTrackingNumber || '').trim() && originalTracking) {
      linkBackfill['waslahReturn.originalTrackingNumber'] = originalTracking;
      waslahReturn.originalTrackingNumber = originalTracking;
    }
    if (!String(order.waslahReturn?.originalWaslahOrderId || '').trim() && order.waslah?.orderId) {
      linkBackfill['waslahReturn.originalWaslahOrderId'] = String(order.waslah.orderId).trim();
      waslahReturn.originalWaslahOrderId = String(order.waslah.orderId).trim();
    }

    if (!persist || !order._id) {
      return {
        order: { ...order, waslahReturn },
        changed: trackingChanged || labelChanged,
        fetched: true,
      };
    }

    if (!trackingChanged && !labelChanged) {
      const touchFields = {
        'waslahReturn.lastStatusSyncAt': syncAt,
        ...linkBackfill,
      };
      ...
      return {
        ...
        changed: Boolean(Object.keys(linkBackfill).length),
        fetched: true,
      };
    }

    const update = {
      'waslahReturn.trackingNumber': waslahReturn.trackingNumber,
      'waslahReturn.processed': waslahReturn.processed,
      'waslahReturn.lastStatusSyncAt': syncAt,
      ...linkBackfill,
    };
    if (nextLabel) update['waslahReturn.labelUrl'] = nextLabel;

    const saved = await Order.findByIdAndUpdate(order._id, { $set: update }, { new: true }).lean();
    return {
      order: {
        ...order,
        waslahReturn: {
          ...(order.waslahReturn || {}),
          ...(saved?.waslahReturn || waslahReturn),
        },
    ...
    console.warn('[waslah-sync] return tracking refresh failed:', error?.message || error);
    return { order, changed: false, error: error?.message || String(error) };
  }
}

async function attachWaslahReturnTracking(result = {}, order = {}, persist = true) {
  const baseOrder = result.order || order;
  if (!String(baseOrder?.waslahReturn?.orderId || order?.waslahReturn?.orderId || '').trim()) {
    return result;
  }

  const returnResult = await syncWaslahReturnTrackingForOrder(baseOrder, { persist });
  let mergedOrder = {
    ...baseOrder,
    waslahReturn: returnResult.order?.waslahReturn || baseOrder.waslahReturn,
    updatedAt: returnResult.order?.updatedAt || baseOrder.updatedAt,
  };

  try {
    const { syncReturnPickupFromWaslahReturn, syncReturnPickupFromFollowUpOrder } = await import('@/lib/storeReturnWorkflow');
    const { getOrderFulfillmentKind } = await import('@/lib/storeReturnLabels');
    await syncReturnPickupFromWaslahReturn(mergedOrder);
    if (getOrderFulfillmentKind(mergedOrder) === 'RETURN') {
      await syncReturnPickupFromFollowUpOrder(mergedOrder);
    }
  } catch (syncError) {
    console.warn('[waslah-sync] return pickup workflow sync failed:', syncError?.message || syncError);
  }

  if (returnResult.error && !result.fetched && result.skipped) {
    return {
      ...result,
      order: mergedOrder,
      error: result.error || returnResult.error,
    };
  }
  ...
  return {
    ...result,
    order: mergedOrder,
    changed: Boolean(result.changed || returnResult.changed),
    returnTrackingChanged: Boolean(returnResult.changed),
    fetched: Boolean(result.fetched || returnResult.fetched),
    // Manual refresh should not look "skipped" when return AWB was refreshed.
    skipped: Boolean(result.skipped) && !returnResult.fetched,
    error: result.error || returnResult.error,
  };
}
...
export async function syncWaslahStatusForOrder(order = {}, { persist = true, force = false } = {}) {
  const outbound = await syncWaslahOutboundStatusForOrder(order, { persist, force });
  return attachWaslahReturnTracking(outbound, order, persist);
}

/**
 * Outbound (sale) Waslah / EMX status sync only.
 */
async function syncWaslahOutboundStatusForOrder(order = {}, { persist = true, force = false } = {}) {
  ...
  const hasOutboundShipment = Boolean(
    order?.waslah?.orderId
    || order?.waslah?.emxTrackingNumber
    || order?.waslah?.trackingNumber
    || order?.waslah?.waslahTrackingNumber
  ...
  // Return-only orders are handled by attachWaslahReturnTracking — skip outbound path.
  ...
  try {
    ...
    if (!waslah) {
      ...
      const update = {
        $set: {
          ...
          'waslah.lastStatusSyncAt': new Date(),
    ...
    update['waslah.lastStatusSyncAt'] = new Date();
    ...
    try {
      const { syncReturnPickupFromWaslahReturn, syncReturnPickupFromFollowUpOrder } = await import('@/lib/storeReturnWorkflow');
      const { getOrderFulfillmentKind } = await import('@/lib/storeReturnLabels');
      if (String(persistedOrder?.waslahReturn?.orderId || '').trim()) {
        await syncReturnPickupFromWaslahReturn(persistedOrder);
      }
